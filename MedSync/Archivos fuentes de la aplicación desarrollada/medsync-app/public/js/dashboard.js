        ...
        if (result.success) {
            ...
            result.data.forEach(section => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="text-center">${section.available_seats !== null ? section.available_seats : section.max_students} / ${section.max_students}</td>
...
window.switchEnrollTab = function(tabName) {
    ...
    tabReg.classList.toggle('active', showRegister);
    tabEnr.classList.toggle('active', !showRegister);
    secReg.style.display = showRegister ? 'block' : 'none';
    secEnr.style.display = showRegister ? 'none' : 'block';
}
...
async function fetchReports() {
    try {
        ...
        if (json.success) {
            json.data.forEach(r => {
                let badgeClass = r.final_score >= 4 ? 'status-open' : (r.final_score >= 3 ? 'status-inprogress' : 'status-closed');
                tbody.innerHTML += `<tr>
                    <td>${r.teacher}</td>
                    <td>${translateCourse(r.course_name)}</td>
                    <td>${r.section_code}</td>
                    <td class="text-center"><span class="status-badge ${badgeClass}">${r.final_score} / 5</span></td>
                    <td class="text-center">${r.total_responses}</td>
                </tr>`;
            });
        }
    } catch (e) {
