        ...
        if (json.success) {
            ...
            json.data.forEach((st, index) => {
                ...
                tr.innerHTML = `
                    <td>${st.dni}</td>
                    <td>${st.full_name}</td>
                    <td style="text-align: center; background: var(--success-bg);">
                        <input type="radio" name="att_${st.student_id}" value="PRESENT" checked>
                    </td>
                    <td style="text-align: center; background: var(--warning-bg);">
                        <input type="radio" name="att_${st.student_id}" value="LATE">
                    </td>
                    <td style="text-align: center; background: var(--danger-bg);">
            ...
            document.querySelectorAll('.calc-grade').forEach(input => {
                input.addEventListener('input', (e) => {
                    ...
                    if (!document.getElementById(`ep_${enrId}`).value && !document.getElementById(`evc_${enrId}`).value && !document.getElementById(`ef_${enrId}`).value) {
                        ...
                        else pfCell.style.color = 'var(--error)';
                    }
                });
            });
        }
...
window.switchTeacherTab = function(tabName) {
    ...
    tabAtt.classList.toggle('active', showAttendance);
    tabGra.classList.toggle('active', !showAttendance);
    secAtt.style.display = showAttendance ? 'block' : 'none';
    secGra.style.display = showAttendance ? 'none' : 'block';
}
