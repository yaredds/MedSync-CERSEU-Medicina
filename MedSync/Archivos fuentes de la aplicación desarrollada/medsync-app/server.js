...
// Obtener cursos matriculados del estudiante (para árbol de sidebar, breadcrumb, progreso semanal e info del curso)
    ...
    try {
        const [rows] = await pool.execute(`
            SELECT
                ...
                (sc.survey_completed_id IS NOT NULL) AS survey_completed
            FROM enrollment e
            JOIN course_section cs ON e.course_section_id = cs.course_section_id
            JOIN course c ON cs.course_id = c.course_id
            JOIN health_area ha ON c.health_area_id = ha.health_area_id
            JOIN teacher t ON cs.teacher_id = t.teacher_id
            LEFT JOIN survey_completed sc ON sc.enrollment_id = e.enrollment_id
            WHERE e.student_id = ?
            ORDER BY ha.name, c.name
...
// Obtener pesos de evaluación (para no hardcodear los porcentajes EC/EF en el frontend)
app.get('/api/assessment-weights', async (req, res) => {
    try {
        const [rows] = await pool.execute('SELECT name, weight_percentage FROM assessment_type');
        res.json({ success: true, data: rows });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'Error al obtener pesos de evaluación.' });
    }
});
...
app.get('/api/student/:student_id/grades', async (req, res) => {
    try {
        const [rows] = await pool.execute(`
            SELECT v.course_name, v.section_code, v.midterm, v.continuous_eval, v.final_exam, v.final_grade, v.status, v.enrollment_id, e.course_section_id
...
app.get('/api/student/:student_id/attendance-details', async (req, res) => {
    try {
        const [rows] = await pool.execute(`
            SELECT s.session_date, a.status, c.name as course_name, cs.section_code, cs.course_section_id
            FROM attendance a
...
app.get('/api/student/:student_id/schedule', async (req, res) => {
    try {
        const [rows] = await pool.execute(`
            SELECT c.name as course_name, cs.section_code, cs.course_section_id, sch.day_of_week, sch.start_time, sch.end_time
            FROM schedule sch
